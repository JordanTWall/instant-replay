...
export const fetchGames = async (teamName: string, season: string): Promise<GameObject[]> => {
  try {
    const response = await axios.get(`https://api.wallwebdevelopment.com/instantreplay/api/games`, {
      params: {
        teamName: teamName,
        season: season,
      },
...
export const fetchGameEvents = async (gameId: number): Promise<GameEvent[]> => {
  try {
    const response = await axios.get<GameEventResponse>(`https://api.wallwebdevelopment.com/instantreplay/api/events`, {
      params: { gameId },
    });
